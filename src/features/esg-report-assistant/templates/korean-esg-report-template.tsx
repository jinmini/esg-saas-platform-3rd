...
const KoreanEsgReportTemplate = React.forwardRef<HTMLDivElement, KoreanEsgReportTemplateProps>(
  ({ reportData, companyName }, ref) => {
    const categorizedData = {
      E: reportData.filter(entry => identifyESGCategorySync(entry.standard.code) === 'E'),
      S: reportData.filter(entry => identifyESGCategorySync(entry.standard.code) === 'S'),
      G: reportData.filter(entry => identifyESGCategorySync(entry.standard.code) === 'G'),
      // General or uncategorized
      ETC: reportData.filter(entry => identifyESGCategorySync(entry.standard.code) === 'unknown'),
    };
