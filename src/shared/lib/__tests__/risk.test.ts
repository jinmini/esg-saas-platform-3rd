...
describe('리스크 유틸리티 함수', () => {
  ...
  describe('getRiskBackgroundColor', () => {
    it('각 리스크 레벨에 대해 올바른 배경색을 반환해야 한다', () => {
      expect(getRiskBackgroundColor('low')).toBe('#d1fae5')
      expect(getRiskBackgroundColor('medium')).toBe('#fef3c7')
      expect(getRiskBackgroundColor('high')).toBe('#fee2e2')
      expect(getRiskBackgroundColor('critical')).toBe('#fca5a5')
    })
  })

  // getRiskLevelText 함수는 현재 구현되지 않음
  ...
  // getESGCategoryWeight, adjustRiskByIndustry 함수들은 현재 구현되지 않음
