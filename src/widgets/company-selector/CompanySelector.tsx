...
interface CompanySelectorProps {
  ...
  isLoading?: boolean;
  onGotoGRIReport?: (companyId: string) => void;
  onGotoNewsAnalysis?: (companyId: string) => void;
  onGotoDetail?: (companyId: string) => void;
}
          ...
          <div className="border rounded-lg p-4 space-y-3 bg-gray-50">
            ...
            <div className="flex space-x-2 pt-2">
              <button 
                ...
                onClick={() => onGotoGRIReport?.(selectedCompany.id)}
              >
                GRI 보고서
              </button>
              <button 
                className="px-3 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors"
                onClick={() => onGotoNewsAnalysis?.(selectedCompany.id)}
              >
                뉴스 분석
              </button>
              <button 
                className="px-3 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors"
                onClick={() => onGotoDetail?.(selectedCompany.id)}
              >
                상세 보기
              </button>
            </div>
